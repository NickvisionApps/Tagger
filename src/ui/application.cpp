...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setVersion("2022.9.1-next");
    m_appInfo.setChangelog("<ul><li>Added the ability to open a folder by dragging one from another application</li><li>Added the 'Fingerprint' property which contains the chromaprint fingerprint of a music file</li><li>Fixed an issue where Tagger may crash on application startup</li><li>Small design tweaks</li></ul>");
