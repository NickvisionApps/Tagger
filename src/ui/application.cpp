...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setChangelog("<ul><li>Tagger now requires clicking the 'Apply' button to save changes to selected tags after inserting/removing album art, converting filenames to tags, or downloading metadata from MusicBrainz</li><li>Changing a file selection without clicking 'Apply' will preserve the changes to the tags, however, if the music folder is changed/reloaded or if the application is closed without clicking 'Apply', the changes will be lost</li><li>Deleting Tags continues to be a permanent action that will take affect as soon as the action is confirmed from the message box</li><li>Added the 'Overwrite Tag with Metadata' option to preferences</li><li>Added the ability to click on the album art in the tag properties pane to insert album art</li><li>Fixed an issue where 'Tag to Filename' was not updating the music files list after completing</li></ul>");
