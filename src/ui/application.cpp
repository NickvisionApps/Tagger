...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setVersion("2022.9.2");
    m_appInfo.setChangelog("<ul><li>Added support for downloading tag metadata from MusicBrainz</li><li>Fixed an issue where Tagger would not allow opening more than about 1024 files</li><li>Fixed an issue where the chromaprint fingerprint contained an extra alien unicode character</li><li>UX improvements</li></ul>");
