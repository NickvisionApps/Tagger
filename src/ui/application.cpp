...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setVersion("2022.10.4-beta2");
    m_appInfo.setChangelog("<ul><li>Added an advanced search function. Type '!' (without quotes) in the search box to activate it and learn more</li><li>Added 'Discard Unapplied Changes' action</li><li>Tagger now remembers user-filled tag properties waiting to be applied</li><li>Improved closing and reloading dialogs</li></ul>");
