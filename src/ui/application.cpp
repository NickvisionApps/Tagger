...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setChangelog("<ul><li>The 'Delete Tag' action must now be applied to be saved to the file</li><li>Fixed an issue where the 'Apply' action would clear the file selection</li></ul>");
