...
Application::Application(const std::string& id, GApplicationFlags flags) : m_adwApp{ adw_application_new(id.c_str(), flags) }
{
    ...
    m_appInfo.setChangelog("<ul><li>Added 'Discard Unapplied Changes' action</li><li>Tagger now remembers user-filled tag properties waiting to be applied</li><li>Improved closing and reloading dialogs</li></ul>");
