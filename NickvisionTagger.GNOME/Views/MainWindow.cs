...
public partial class MainWindow : Adw.ApplicationWindow
{
    ...
    {
        var dialog = new MessageDialog(this, _controller.AppInfo.ID, _("Corrupted Files Found"), _("This music folder contains music files whos tags are corrupted. What would you like to do?"), _("Ignore Files"), _("Clear Tags"));
        dialog.OnResponse += async (sender, e) =>
        {
            if(dialog.Response == MessageDialogResponse.Destructive)
            {
                SetLoadingState(_("Loading music files from folder..."));
                await _controller.ClearCorruptedTagsAsync();
                await _controller.ReloadFolderAsync();
            }
            dialog.Destroy();
        };
