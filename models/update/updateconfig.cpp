...
namespace NickvisionTagger::Models::Update
{
    ...
    std::optional<UpdateConfig> UpdateConfig::loadFromUrl(const std::string& url)
    {
        ...
        std::ofstream updateConfigFileOut(std::string(getpwuid(getuid())->pw_dir) + "/.config/Nickvision/NickvisionTagger/UpdateConfig.json");
        ...
        std::ifstream updateConfigFileIn(std::string(getpwuid(getuid())->pw_dir) + "/.config/Nickvision/NickvisionTagger/UpdateConfig.json");
